...
homebrew_command_not_found_handle () {
	...
	then
		[[ -n "${BASH_VERSION}" ]] && TEXTDOMAIN=command-not-found echo $"${cmd}: command not found"
		[[ -n "${ZSH_VERSION}" ]] && is-at-least "5.2" "${ZSH_VERSION}" && echo "zsh: command not found: ${cmd}" >&2
		return 127
	fi
...
if ! (unalias rg 2>/dev/null; command -v rg) >/dev/null 2>&1; then
  alias rg='/Users/sev/.local/share/claude/versions/2.1.19 --ripgrep'
fi
export PATH=/Users/sev/bin\:/Users/sev/.local/bin\:/opt/homebrew/bin\:/opt/homebrew/sbin\:/usr/local/bin\:/System/Cryptexes/App/usr/bin\:/usr/bin\:/bin\:/usr/sbin\:/sbin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/local/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/appleinternal/bin\:/Library/Apple/usr/bin\:/usr/local/MacGPG2/bin\:/Applications/iTerm.app/Contents/Resources/utilities
